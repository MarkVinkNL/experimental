...
function setup() {
  ...
  gui.add(settings, "wispGravity", -2.0, 2.0, 0.1).name("Wisp Gravity");

  gui.add(settings, "wispLifetimeMin", 10, 300, 10).name("Wisp Lifetime Min");

  gui.add(settings, "wispLifetimeMax", 10, 500, 10).name("Wisp Lifetime Max");

  gui.add(settings, "spawnOnMouseOnly").name("Spawn on Mouse Over");
...
class Bubble {
  ...
  show() {
    ...
    // Check if we should spawn a wisp
    let shouldSpawn = random() < settings.wispSpawnRate;
    if (settings.spawnOnMouseOnly) {
      // Only spawn if mouse is over the bubble
      let d = dist(mouseX, mouseY, this.x, this.y);
      shouldSpawn = shouldSpawn && d < this.d / 2;
    }
...
class Wisp {
  constructor(x, y, d, r) {
    ...
    this.d = d;
    this.timer = 0;
    this.endTime = int(random(settings.wispLifetimeMin, settings.wispLifetimeMax));
    this.ang = random(TAU);
    this.r = r;
    this.originX = this.x;
    this.originY = this.y;
    ...
    this.originD = d;
    this.isDead = false;
    // Reverse gravity - upward drift
    this.gravity = settings.wispGravity; // Negative for upward movement
  }
  ...
  move() {
    ...
    if (0 < this.timer && this.timer < this.endTime) {
      ...
      // Apply gravity as cumulative offset to base position
      let baseY = lerp(this.originY, this.targetY, easeOutQuint(n));
      // Use squared formula for acceleration effect
      this.y = baseY + this.gravity * this.timer * this.timer * 0.01;

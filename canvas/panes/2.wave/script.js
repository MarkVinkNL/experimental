...
function getWaveRotation(nowMs, paneX, paneY) {
  ...
  const dx = paneX - state.wave.x;
  const dy = paneY - state.wave.y;
  const distance = Math.hypot(dx, dy);
  const arrivalMs = (distance / config.waveSpeed) * 1000;
  const localElapsed = nowMs - state.wave.startMs - arrivalMs;
  ...
  return flipPhase * ((config.waveFlipAngleDeg * Math.PI) / 180);
}
...
function render(nowMs = performance.now()) {
  if (state.wave && nowMs > state.wave.endMs) {
    state.wave = null;
  }
